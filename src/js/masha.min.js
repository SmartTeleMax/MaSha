...
this.options.marker),this.marker.setAttribute("href","#"),document.body.appendChild(this.marker))):this.marker=this.options.marker;if("string"!=typeof this.options.regexp)throw"regexp is set as string";this.regexp=RegExp(this.options.regexp,"ig");var c=this;this.selectable&&(this.isIgnored=this.constructIgnored(this.options.ignored),this.options.selectMessage&&this.initMessage(),this.enumerateElements(),"ontouchstart"in window||window.DocumentTouch&&document instanceof DocumentTouch?g(this.selectable,
"touchend",function(){window.setTimeout(function(){var b=window.getSelection();if(b.rangeCount){b=b.getRangeAt(0).getClientRects();if(b=b[b.length-1])var c={x:b.left+b.width+document.body.scrollLeft,y:b.top+b.height/2+document.body.scrollTop};a(c)}},1)}):g(this.selectable,"mouseup",function(b){var c=D(b);window.setTimeout(function(){a(c)},1)}),g(this.marker,"click",b),g(this.marker,"touchend",b),g(document,"click",function(a){(a.target||a.srcElement)!=c.marker&&s(c.marker,"show")}),this.options.enableHaschange&&
this.options.location.addHashchange(function(){if(c.lastHash!=c.options.location.getHash()){var a=[],b;for(b in c.ranges)a.push(b);c.deleteSelections(a);c.readHash()}}),this.readHash())},openShareWindow:function(a){window.open(a,"","status=no,toolbar=no,menubar=no,width=800,height=400")},getMarkerCoords:function(a,b){return{x:b.x+5,y:b.y-33}},deleteSelections:function(a){for(var b=[],c=a.length;c--;){var d=a[c],e=v(this.selectable,d),f=t(e[e.length-1],"closewrap");f.parentNode.removeChild(f);this.removeTextSelection(e);
...
if(3==a.nodeType&&null!=a.data.match(e))return{_container:a,_offset:d*a.data.length}}a=a.parentNode}return null},prevNode:function(a,b){return this._siblingNode(a,"previous","last",1,b)},nextNode:function(a,b){return this._siblingNode(a,"next","first",0,b)},wordCount:function(a){var b=0;if(3==a.nodeType)(a=a.nodeValue.match(this.regexp))&&(b+=a.length);else if(a.childNodes&&a.childNodes.length){a=o(a);for(i=a.length;i--;)b+=a[i].nodeValue.match(this.regexp).length}return b},words:function(a,b,c){1==
